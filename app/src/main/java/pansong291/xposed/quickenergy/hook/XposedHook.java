...
public class XposedHook implements IXposedHookLoadPackage {
    ...
    public static boolean isIsHooked() {
        return isHooked;
    }
    ...
    public static void initHandler(Boolean force) {
        try {
            if (context != null) {
                stopHandler();
                if (!isInit || force) {
    ...
    private void hookService() {
            ...
            XposedHelpers.findAndHookMethod("com.alipay.mobile.quinox.LauncherActivity", classLoader,
                    "onResume", new XC_MethodHook() {
                        ...
                        protected void afterHookedMethod(MethodHookParam param) {
                            ...
                            if (!isInit) {
                                initHandler(false);
                                Log.i(TAG, "Activity initHandler");
                                return;
                            }
                            String targetUid = RpcUtil.getUserId();
                            if (targetUid == null) {
        ...
        try {
            ...
            Log.i(TAG, "hook rpc request successfully");
        } catch (Throwable t) {
            Log.i(TAG, "hook rpc request err:");
            Log.printStackTrace(TAG, t);
        }
        ...
        try {
            ...
            Log.i(TAG, "hook rpc response successfully");
        } catch (Throwable t) {
            Log.i(TAG, "hook rpc response err:");
            Log.printStackTrace(TAG, t);
        }
